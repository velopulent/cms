...
impl CollectionRepository for PostgresCollectionRepository {
    ...
    async fn get_content_for_migration(&self, collection_id: &str) -> Result<Vec<Content>, RepositoryError> {
        let result = sqlx::query_as::<_, Content>(
            "SELECT id, site_id, collection_id, data::text as data, slug, status, created_at::text as created_at, updated_at::text as updated_at, published_at::text as published_at FROM content WHERE collection_id = $1",
        )
        .bind(collection_id)
        .fetch_all(&self.pool)
        .await?;
