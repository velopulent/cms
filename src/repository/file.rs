...
pub async fn get_by_id_any(pool: &SqlitePool, id: &str) -> Result<Option<File>, sqlx::Error> {
    sqlx::query_as::<_, File>(
        "SELECT id, site_id, filename, original_name, mime_type, size, storage_provider, storage_key, thumbnail_key, width, height, deleted_at, created_by, created_at
         FROM files WHERE id = ?",
    )
...
pub async fn list(pool: &SqlitePool, params: ListFilesParams<'_>) -> Result<FileListResult, sqlx::Error> {
...
pub async fn restore(pool: &SqlitePool, id: &str, site_id: &str) -> Result<u64, sqlx::Error> {
    let result = sqlx::query(
...
pub async fn get_deleted_by_ids(
    pool: &SqlitePool,
    site_id: &str,
    ids: &[String],
...
pub async fn batch_permanent_delete(
    pool: &SqlitePool,
    site_id: &str,
    ids: &[String],
) -> Result<u64, sqlx::Error> {
...
pub async fn get_references_for_site(
    pool: &SqlitePool,
    file_id: &str,
    site_id: &str,
) -> Result<Vec<FileReference>, sqlx::Error> {
