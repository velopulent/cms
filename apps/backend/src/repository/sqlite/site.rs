...
impl SiteRepository for SqliteSiteRepository {
    async fn list_all(&self) -> Result<Vec<Site>, RepositoryError> {
        let result = sqlx::query_as::<_, Site>(
            "SELECT id, name, storage_provider, storage_profile_id, created_by, created_at, updated_at FROM sites ORDER BY name",
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn list_for_user(&self, user_id: &str) -> Result<Vec<SiteWithRole>, RepositoryError> {
        let result = sqlx::query_as::<_, SiteWithRole>(
            "SELECT s.id, s.name, s.storage_provider, s.storage_profile_id, s.created_by, s.created_at, s.updated_at, sm.role
             FROM sites s
    ...
    async fn get_by_id(&self, id: &str) -> Result<Option<Site>, RepositoryError> {
        let result = sqlx::query_as::<_, Site>(
            "SELECT id, name, storage_provider, storage_profile_id, created_by, created_at, updated_at FROM sites WHERE id = ?",
        )
        .bind(id)
    ...
    async fn list_members(&self, site_id: &str) -> Result<Vec<SiteMember>, RepositoryError> {
        let result = sqlx::query_as::<_, SiteMember>(
            "SELECT sm.id, sm.site_id, sm.user_id, u.name, u.email, sm.role, sm.created_at
