...
impl WebhookRepository for MysqlWebhookRepository {
    async fn list_for_site(&self, site_id: &str) -> Result<Vec<SiteWebhook>, RepositoryError> {
        let result = sqlx::query_as::<_, SiteWebhook>(
            "SELECT id, site_id, label, url, headers_encrypted, enabled, created_by, CAST(created_at AS CHAR) AS created_at, CAST(updated_at AS CHAR) AS updated_at FROM site_webhooks WHERE site_id = ? ORDER BY created_at",
        )
        .bind(site_id)
        .fetch_all(&self.pool)
        .await?;
    ...
    async fn get_by_id(&self, id: &str, site_id: &str) -> Result<Option<SiteWebhook>, RepositoryError> {
        let result = sqlx::query_as::<_, SiteWebhook>(
            "SELECT id, site_id, label, url, headers_encrypted, enabled, created_by, CAST(created_at AS CHAR) AS created_at, CAST(updated_at AS CHAR) AS updated_at FROM site_webhooks WHERE id = ? AND site_id = ?",
        )
        .bind(id)
        .bind(site_id)
...
impl MysqlWebhookRepository {
    async fn get_by_id_unscoped(&self, id: &str) -> Result<Option<SiteWebhook>, RepositoryError> {
        sqlx::query_as::<_, SiteWebhook>(
            "SELECT id, site_id, label, url, headers_encrypted, enabled, created_by, CAST(created_at AS CHAR) AS created_at, CAST(updated_at AS CHAR) AS updated_at FROM site_webhooks WHERE id = ?",
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await
